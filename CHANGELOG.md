...
Hand-maintained notes for this personalized fork, layered on top of the upstream releases below (which remain auto-generated by release-please and will regenerate on `npm run update`).

- **2026-06 to 2026-07** — Retargeted for a Hyderabad-based DevOps/Platform/SRE/MLOps/Cloud job search:
  - Rebuilt `portals.yml`: curated India product companies + GCCs (Harness, GitLab, Grafana Labs, Databricks, Datadog, Cloudflare, MongoDB, Rubrik, Postman, Atlan, Sarvam AI, Groww, Druva, Baseten, Amgen, Novartis, Sanofi, Eli Lilly, UnitedHealth/Optum, Micron, Vanguard, DAZN, Microsoft, Google, Amazon, Apple, Meta, Salesforce, Qualcomm, Uber, ServiceNow, IBM, Oracle, VMware, Cisco, Intel, Broadcom, Ericsson, HSBC, Bank of America, State Street, US Bank, UBS, FactSet, Broadridge, D.E. Shaw, Invesco, Roche, Bayer, Bristol Myers Squibb, HCA, PepsiCo, McDonald's, Costco, L'Oréal, Bosch, Ford, Honeywell, Medtronic, MSD, Cigna/Evernorth, Sandoz, ABB, Johnson Controls, Nike, Rolls-Royce, SimCorp, Zelis, Calyx, ServiceTitan, Kantar, RSM); explicitly excludes IT-services/consulting firms. Enabled remote job boards: RemoteOK, Remotive, Working Nomads.
  - Added a strict location filter: Hyderabad/Telangana (any work mode) or fully-remote-only elsewhere, enforced in both the scanner (`portals.yml`) and evaluation scoring (`modes/_profile.md`).
  - Retargeted archetypes to DevOps / SRE / Platform Engineer / MLOps / Cloud Engineer; comp guidance switched to INR total-CTC plus remote-USD.
  - Verified and wired 20+ Workday-hosted company boards for zero-token scanning (sourced via the reverse-discovery dataset and CXS-endpoint verification).

## Upstream Changelog

Auto-generated by release-please for the upstream [santifer/career-ops](https://github.com/santifer/career-ops) project.
